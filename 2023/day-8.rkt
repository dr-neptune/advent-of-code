...
;; (define input (fetch-aoc-input (find-session) 2023 6))
...
use in-cycle on the initial RL commands and loop through a hash map that points to the next node
...
(define (get-node-trip starting-value [ending-value "ZZZ"])
  (let ([curr-node starting-value])
    (for/sum ([dir (in-cycle (string->list instructions))]
              #:break (equal? curr-node ending-value))
...
;; part 2
(define (get-node-trip starting-value [ending-value #\Z])
  (let ([curr-node starting-value])
    (for/sum ([dir (in-cycle (string->list instructions))]
              #:break (equal? (last (string->list curr-node)) ending-value))
      (let ([nxt (first (get-node curr-node dir))])
        (set! curr-node nxt) 1))))
